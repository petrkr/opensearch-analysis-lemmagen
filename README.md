# LemmaGen Analysis for OpenSearch

The LemmaGen Analysis plugin provides [jLemmaGen lemmatizer](https://github.com/hlavki/jlemmagen) as OpenSearch token filter.

This repository is a fork of the original Elasticsearch plugin by [Vojtěch Hýža](https://github.com/vhyza/elasticsearch-analysis-lemmagen), adapted for OpenSearch.
...
This plugin does not provide built-in lexicons. There is a separate [lemmagen-lexicons repository](https://github.com/vhyza/lemmagen-lexicons) with them.

Copy desired lexicon(s) from [lemmagen-lexicons repository](https://github.com/vhyza/lemmagen-lexicons) into OpenSearch `config/lemmagen` directory (keep the `.lem` extension).
...
After plugin installation and OpenSearch restart you should see in logs something like:

```bash
[INFO ][o.o.p.PluginsService] loaded plugin [opensearch-analysis-lemmagen]
```
...
* `lexicon_path` - relative path to the lexicon file from OpenSearch config directory
...
[Vojtěch Hýža](https://github.com/vhyza/elasticsearch-analysis-lemmagen) for the original Elasticsearch plugin
